...
public class CloudSim {
     ...
     * indicates that {@link gridsim.CloudSimShutdown} first waits for all user
     * entities's END_OF_SIMULATION signal before issuing terminate signal to
     * other entities
    ...
    public static boolean runClockTick() {
        ...
        if (future.size() > 0) {
            // ATAKAN: Check if all remaining events are cache operations
            boolean allCache = true;
            Iterator<SimEvent> fcit = future.iterator();
            while (fcit.hasNext()) {
                SimEvent next = fcit.next();
                if (next.getTag() != CloudSimTags.CHECK_DEMAND_FOR_CACHES) {
                    allCache = false;
                    break;
                }
            }
            if(allCache && CloudSim.clock()>1000){ //Same as else
                running = false;
